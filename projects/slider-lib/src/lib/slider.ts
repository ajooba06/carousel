...
export class Slider {
    ...
    handleLeftSlide() {
        ...
        if (!isLastSlide) {
            ...
            if (this.isLazyLoad) {
                this.isHolderLocked = this.preliminarySlideCounter > this.overflowCellsLimit;

                if (this.detectContainerUnlock()) {
                    this.isHolderLocked = false;
                }
            }
        } else {
            this.slideLength = 0;
